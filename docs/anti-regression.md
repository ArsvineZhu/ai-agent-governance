...
- **6-phase operating lifecycle** — every development task runs Understand → Plan → Implement → Validate → Synchronize → Report; medium/large changes require a `docs/plans/TASK_*.md` before any code; changing code without updating project knowledge is forbidden
...
- **六阶段操作生命周期** — 每个开发任务走 Understand → Plan → Implement → Validate → Synchronize → Report；中大型改动必须先建 `docs/plans/TASK_*.md` 再动代码；"只改代码不更新项目知识"被禁止
