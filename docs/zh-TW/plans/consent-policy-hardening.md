...
清除三層補丁屎山，把確認政策從「每步確認 + 例外補丁」重構為單一原則——**提交前回顯完整 git 命令序列，確認一次後執行**。使用者說 push 後，展示將要執行的 add 檔案清單、commit 訊息（類型含在訊息前綴）、push 目標分支，使用者確認一次即執行完。計劃批准降級為意圖對齊；規模分級降級為決定是否寫計劃文件。政策曾被其作者多次違反（本會話真實事故），重寫目標是一次清掉舊框架，不留補丁。
