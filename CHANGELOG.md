...
- **INIT scripted generator (Phase A)** — `scripts/generate-governance.js`: deterministic, zero-dependency Node generator that produces the Phase A static skeleton (5 rules files, AGENTS.md with resolved placeholders, CHANGELOG, README skeleton, feature placeholder, ARCHITECTURE, development plan, .governance state files); reads `references/init-spec.json` for artifact specifications; supports `--dry-run`, `--json`, `--phase A|B|C`; same inputs produce byte-identical outputs (determinism verified by tests). SKILL.md INIT can now delegate file writing to this script, keeping only confirmation gates for the agent.
