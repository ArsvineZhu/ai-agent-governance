...
- **L3 audit mode still wrote the manifest** — the manifest is generated after the artifact loop and never consulted the audit flag, so `--maturity LEVEL_3_PRODUCTION` (documented as report-only) silently wrote `.governance/manifest.json` into a production repo. Now honours audit mode; `--force-l3` still writes. Found by the full-audit review of the change set.
...
- **consent-cluster check false negative: a keyword reference masqueraded as a definition** — the `--gate` consent check matched the bare phrases `Exception A` / `例外 A` / `例外一`, but those strings also appear in *reference* lines (e.g. git.policy.md's "此时适用下方例外 A。"). Deleting the actual definition while leaving such a reference made the gate report `gatePass=true` — a leftover reference was treated as a live rule. Reproduced in both a synthetic fixture and against the real `git.policy.md` (strip the definition, keep the reference line → previously passed, now correctly fails and names the file). Fixed by tightening the A/B markers to **definition-title** patterns (em-dash headings: `Exception A —` / `例外 A ——` / `例外一 ——`), which excludes reference lines while still matching all four real sync points. Added a regression test (definition removed, reference retained → `--gate` exits 1) and confirmed the tightened regexes match the four real files and neither cross-match A↔B. Verified by the full suite and a real-file regression run.
