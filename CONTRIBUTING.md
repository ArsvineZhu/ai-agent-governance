...
The test suite covers: empty project (exit 1), full default structure (exit 0), custom doc root via manifest (manifest mode), missing governance_version (exit 1), `--json` output, `--help`, no legacy `.agent` leakage, optional `validation.json`, release planning (SemVer classification: docs/refactor → patch, CLI → minor, deleted API → major, uncertainty → clarification), and the approval gate (unapproved → no tag, approved → annotated tag). CI runs it on every push/PR.
...
测试套件覆盖：空项目（exit 1）、完整默认结构（exit 0）、自定义文档根经 manifest（manifest 模式）、缺 governance_version（exit 1）、`--json` 输出、`--help`、无 `.agent` 残留、`validation.json` 可选、发布规划（SemVer 分类：docs/重构 → patch、CLI 命令 → minor、删除公开 API → major、不确定性 → 澄清）与审批门禁（未批准 → 无 tag，批准 → 创建 annotated tag）。CI 每次 push/PR 运行。
